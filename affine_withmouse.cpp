	...
	do
	{
		...
		if (LB_FLAG == 2 ) {
			...
			for (int i = 0; i < 41; i++)
			{
				...
				srcTri[1] = Point2f(src_img.cols - 1 - (src_img.cols - (select.x + select.width))*i / 40.0, select.y * i / 40.0);
				srcTri[2] = Point2f(select.x * i / 40.0, src_img.rows - 1 - (src_img.rows - (select.y + select.height))*i / 40.0);
